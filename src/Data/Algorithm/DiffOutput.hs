-- Enable PLE (Proof by Logical Evaluation) so that reflected predicates like
-- 'coherentDiff' are automatically unfolded when verifying refinement types.
-- Without this, LH cannot prove obligations like @coherentDiff (Both x x) = True@
